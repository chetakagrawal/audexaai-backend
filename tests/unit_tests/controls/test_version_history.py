"""Unit tests for control version history.
...
from services.controls_service import create_control, delete_control, update_control


@pytest.mark.asyncio
async def test_update_control_captures_snapshot(db_session: AsyncSession):
    """Test: Updating a control captures a snapshot in entity_versions."""
    # Setup
    ...
    # Create control (row_version=1)
    from models.control import ControlCreate
    ...
    original_frequency = control.frequency
    original_row_version = control.row_version
    
    # Update control (should trigger snapshot)
    from models.control import ControlBase
    update_payload = ControlBase(
        control_code="AC-001",
        name="Updated Name",
    ...
    result = await db_session.execute(
        text("""
            ...
            WHERE tenant_id = :tenant_id
              AND entity_type = 'controls'
              AND entity_id = :entity_id
            ORDER BY version_num DESC
        """),
        {"tenant_id": str(tenant.id), "entity_id": str(control.id)},
    )
    versions = result.fetchall()
    
    assert len(versions) == 1, "Should have one snapshot"
    version = versions[0]
    assert version.entity_type == "controls"
    assert version.entity_id == control.id
    ...
    assert data["frequency"] == original_frequency, "Snapshot should contain OLD frequency"
    assert data["row_version"] == original_row_version, "Snapshot should contain OLD row_version"
    
    # Verify current control has new values
...
async def test_soft_delete_captures_delete_snapshot(db_session: AsyncSession):
    """Test: Soft deleting a control captures a DELETE snapshot."""
    # Setup
    tenant = Tenant(id=uuid4(), name="Test Tenant", slug="test-tenant", status="active")
    db_session.add(tenant)
    await db_session.flush()
    ...
    pre_delete_row_version = control.row_version  # Should be 2
...
async def test_version_history_tenant_isolation(db_session: AsyncSession):
    ...
    control_a = await create_control(
        db_session,
    ...
    control_b = await create_control(
        db_session,
    ...
    # Update both controls to create snapshots
    await update_control(
        db_session,
        membership_ctx=membership_ctx_a,
    ...
    # Query versions for tenant_a - should only see control_a's versions
    ...
    assert versions_a[0].entity_id == control_a.id
    
    # Query versions for tenant_b - should only see control_b's versions
    ...
    assert versions_b[0].entity_id == control_b.id
